...
public:
	...
	// Find a named value (globally or in the current table). Return true if the value is found, in which 
	// case getValue() may be called (and any const char* returned by getValue() is invalid).
	bool find(const char* _name);

	// Get the next value in the current table. Return true if not the end of the table, in which case 
	// getValue() may be called (and any const char* returned by getValue() is invalid).
	bool next();
	
	// Get the type of the current value.
	ValueType getType() const;

	// Get the current value. tType is expected to match the type of the current value exactly (i.e. 
	// getValue<int>() must be called only if the value type is kNumber). 
	// \note Ptr returned by getValue<const char*> is only valid until the next call to find() or next().
