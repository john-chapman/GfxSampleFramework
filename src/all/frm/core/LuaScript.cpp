...
#include <lua/lauxlib.h>
	...
	#undef FRM_STRICT_ASSERT
	#define FRM_STRICT_ASSERT(x) FRM_ASSERT(x)
...
#define luaAssert(_call) \
	if ((m_err = (_call)) != LUA_OK) { \
		FRM_LOG_ERR("Lua error: %s", lua_tostring(m_state, -1)); \
		lua_pop(m_state, 1); \
	}
...
static void* lua_alloc(void* _ud, void* _ptr, size_t _osize, size_t _nsize)
{
	...
	return FRM_REALLOC(_ptr, _nsize);
}

static int lua_panic(lua_State* _L)
{
	FRM_LOG_ERR("Lua panic: %s", lua_tostring(_L, -1));
	FRM_ASSERT(false);
	return 0; // call abort()
}
...
bool LuaScript::enterTable()
{
	...
	++m_currentTable;
	FRM_ASSERT(m_currentTable != kMaxTableDepth);
...
void LuaScript::leaveTable()
{
	...
	popToCurrentTable();
	--m_currentTable;
	FRM_ASSERT(lua_type(m_state, -1) == LUA_TTABLE); // stack top should be a table
}
...
namespace {
 // need to handle int/float types seperately to avoid precision issues e.g. with string hashes
	...
	tType getValue(lua_State* _L, frm::internal::IntT)
	{
		return (tType)lua_tointeger(_L, -1);	
	}
	template <typename tType>
	tType getValue(lua_State* _L, frm::internal::FloatT)
	{
...
#define LuaScript_getValue_Number(_type, _enum) \
	template <> _type LuaScript::getValue<_type>(int _i) const { \
		...
		if (!lua_isnumber(m_state, -1)) \
			FRM_LOG_ERR("LuaScript::getValue<%s>(%d): not a number", frm::DataTypeString(_enum), _i); \
		auto ret = ::getValue<_type>(m_state, FRM_TRAITS_FAMILY(_type)); \
		if (needPop) \
			lua_pop(m_state, 1); \
		return ret; \
	}
FRM_DataType_decl(LuaScript_getValue_Number)
...
namespace {
	template <typename tType>
	void setValue(lua_State* _L, tType _value, frm::internal::IntT)
	{
		lua_pushinteger(_L, (lua_Integer)_value);	
	}
	template <typename tType>
	void setValue(lua_State* _L, tType _value, frm::internal::FloatT)
	{
...
#define LuaScript_setValue_Number_i(_type, _enum) \
	template <> void LuaScript::setValue<_type>(_type _value, int _i) { \
		if (m_currentTable == 1) { \
			FRM_LOG_ERR("LuaScript::setValue<%s>(%g, %d): not in a table", frm::DataTypeString(_enum), (lua_Number)_value, _i); \
			return; \
		} \
		if (_i == 0 && m_tableIndex[m_currentTable] == 0) { \
			FRM_LOG_ERR("LuaScript::setValue<%s>(%g, %d): stack empty", frm::DataTypeString(_enum), (lua_Number)_value, _i); \
			return; \
		} \
		::setValue<_type>(m_state, _value, FRM_TRAITS_FAMILY(_type)); \
		setValue(_i); \
	}
FRM_DataType_decl(LuaScript_setValue_Number_i)
...
FRM_DataType_decl(LuaScript_setValue_Number_name)
...
{
	FRM_AUTOTIMER_DBG("LuaScript::execute() %s", (const char*)m_name);
...
int LuaScript::call()
{
	FRM_AUTOTIMER_DBG("LuaScript::call() %s", (const char*)m_name);
...
FRM_DataType_decl(LuaScript_pushValue_Number)
...
FRM_DataType_decl(LuaScript_popValue_Number)
...
{
	FRM_AUTOTIMER_DBG("LuaScript %s", (const char*)m_name);
	m_name = _name;
...
void LuaScript::popAll()
{
	...
	n = FRM_MAX(0, n - 1); // we keep the script chunk on the bottom of the stack, see execute()
	lua_pop(m_state, n);
}

void LuaScript::setValue(int _i)
{
	FRM_STRICT_ASSERT(m_currentTable != 1);
	_i = _i ? _i : m_tableIndex[m_currentTable];
...
bool LuaScript::gotoIndex(int _i) const
{
	if (_i > 0) {
		...
		if (_i > m_tableLength[m_currentTable]) {
			FRM_LOG_ERR("LuaScript::gotoIndex(%d): index out of bounds (table length = %d)", _i, m_tableLength[m_currentTable]);
			return false;
		}
