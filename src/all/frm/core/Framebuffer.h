...
public:
	...
	// texture (use attachLayer() or attachFace()). _texture may be 0 in which case the attachment 
	// is removed.
	...
	// an array or 3d texture. _texture may be 0 in which case the attachment is removed.
	...
	// Return the framebuffer status, which should be GL_FRAMEBUFFER_COMPLETE in order for the 
...
private:
	...
	// Convert an attachment enum (e.g. GL_COLOR_ATTACHMENT0) to an index into the attachment list.
